...
def result_content_is_stale(
    result_meta: dict[str, Any] | None,
    *,
    paper_content_hash: str | None,
) -> bool:
    """True when a non-NULL input_content_hash disagrees with the paper hash."""
    if not result_meta:
        return False
    result_hash = result_meta.get("input_content_hash")
    if result_hash is None:
        return False
    if paper_content_hash is None:
        return False
    return str(result_hash) != str(paper_content_hash)
...
def derive_quality_status(
    *,
    ...
    Vocabulary: scored | failed | pending | not_selected | skipped | stale_content.
    """
    if has_current_quality_row:
        return "scored", "scored_quality_result_current_versions"

    if content_stale:
        return "stale_content", "stale_content:input_content_hash_mismatch"
